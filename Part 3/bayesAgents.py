...
def constructBayesNet(gameState):
    ...
    variableDomainsDict[HOUSE_VARS[1]] = HOUSE_VALS
    variableDomainsDict[HOUSE_VARS[0]] = HOUSE_VALS
    top = [X_POS_VAR, Y_POS_VAR]
    mid = [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR]
    ...
    for i in top: #edges entre les deux premiers niveaux
        for j in mid:
            edge = (i,j)
            edges.append(edge)
    
    for j in mid: #edges entre le deuxieme niveau et toutes les observations
        for k in obsVars:
            edge = (j,k)
            edges.append(edge)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    assignDict = factor.getAllPossibleAssignmentDicts()
    maxProb = 0
    maxAssign = {}
    for elem in assignDict:
        value = factor.getProbability(elem)
        if value > maxProb:
            maxProb=value
            maxAssign=elem
    return maxAssign[FOOD_HOUSE_VAR]
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        probLeft = factor.getProbability(left)
        

        right = evidence
        right[FOOD_HOUSE_VAR] = TOP_RIGHT_VAL
        right[GHOST_HOUSE_VAR] = TOP_LEFT_VAL
        probRight = factor.getProbability(right)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        obsProb = self.getExplorationProbsAndOutcomes(evidence)
        for prob, obs in obsProb:
            values = self.computeEnterValues(obs, enterEliminationOrder)
            maxValue = max(values)
            expectedValue += prob * maxValue
        "*** END YOUR CODE HERE ***"

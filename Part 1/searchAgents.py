...
class CornersProblem(search.SearchProblem):
    ...
    def getNextState(self, state, action):
        ...
        if (nextx, nexty) in self.corners:
            ...
            if idx == 0:
                corners = (True, state[1][1], state[1][2], state[1][3])
            elif idx == 1:
                corners = (state[1][0], True, state[1][2], state[1][3])
            elif idx == 2:
                corners = (state[1][0], state[1][1], True, state[1][3])
            elif idx == 3:
                corners = (state[1][0], state[1][1], state[1][2],True)
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    #l'heuristic est définie comme étant la distance de manhattan maximal des corners restants
    x, y = state[0]
    distances = []
    if not state[1][0]:
        d1 = abs(x - corners[0][0]) + abs(y-corners[0][1])
        distances.append(d1)
    if not state[1][1]:
        d2 = abs(x - corners[1][0]) + abs(y-corners[1][1])
        distances.append(d2)
    if not state[1][2]:
        d3 = abs(x - corners[2][0]) + abs(y-corners[2][1])
        distances.append(d3)
    if not state[1][3]:
        d4 = abs(x - corners[3][0]) + abs(y-corners[3][1])  + walls[y:corners[3][1]].count(True)
        distances.append(d4)
    if len(distances) > 0:
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    #l'heuristic est définie comme étant la somme de la distance entre le food le plus proche et la distance(de manhattan)entre les deux food les plus éloignés

    foods = foodGrid.asList()

    if problem.isGoalState(state):
        return 0
    distanceCloseFood = 999999
    distanceFarthestFood = 0
    for food in foods:
        distance = mazeDistance(position, food, problem.startingGameState)
        if distance < distanceCloseFood:
            distanceCloseFood = distance
        for food2 in foods:
            distance = abs(food[0] - food2[0]) + abs(food[1] - food2[1])
            if distance > distanceFarthestFood:
                distanceFarthestFood = distance    
    return distanceCloseFood+distanceFarthestFood 
